...
#define RWM_CONFIGURE_DELAY_USEC 100000
               ...
               void *bindarg)
{
    ...
    window = arg;
    client = window->child;
    XSync(client->app->display, True);
    client->stateflags |= R_WINDOW_DESTROYED_FLAG;
    XUnmapWindow(window->app->display, client->id);
    XDestroyWindow(window->app->display, client->id);
              ...
              void *bindarg)
{
    ...
    client = window->child;
    XSync(client->app->display, True);
    XUnmapWindow(client->app->display, client->id);
    client->stateflags |= R_WINDOW_DESTROYED_FLAG;
                    ...
                    void *bindarg)
{
    ...
    if (!(window->stateflags & R_WINDOW_DESTROYED_FLAG)) {
        ...
        if (!(window->stateflags & R_WINDOW_MOVE_FLAG)) {
            ...
            if (frame->app->flags & RWM_CLICK_FOCUS_FLAG) {
                if (window->child) {
                    Rwm_set_input_focus(window->child, event->xbutton.time);
                }
            }
              ...
              void *bindarg)
{
    ...
    Rwm_move_window(frame,
                    winx,
                    winy);
    if (wm->optflags & RWM_SYNCHRONOUS_MOVE_FLAG) {
                 ...
                 void *bindarg)
{
    ...
    if (wm->optflags & RWM_SYNCHRONOUS_RESIZE_FLAG) {
        gettimeofday(&curtv, NULL);
        if (TIMEVAL_CMP_USEC(&Rwm_action_info.tv, &curtv)
            > RWM_CONFIGURE_DELAY_USEC) {
            Rwm_send_configure(client);
